...
import { useLazyGetLimitedProductsQuery, useLazyGetProductByTitleQuery } from '@/store/products/products.api';
...
function useProducts(searchValue: string, limitCount: number): IUseSearchProducts {
  const { getCountPages, setTotalPages, currentPage, limit, pagesArray, changePage, setCurrentPage } =
    usePagination(limitCount);
  const [getProducts, { data: initProducts, isLoading, isFetching, isError }] = useLazyGetLimitedProductsQuery();
  const [getSearchProducts, { data: searchProducts, isLoading: isSearchLoading, isFetching: isSearchFetching }] =
